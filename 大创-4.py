...
from sympy import symbols, Eq, simplify, solve_linear_system, Matrix
from sympy.core.symbol import Symbol  # 导入Symbol类用于类型检查
        ...
        for i, var in enumerate(variables["b"]):
            ...
            with cols[i % 3]:
                ...
                val = get_input_streamlit(
                    ...
                    var_name=var,
                    key=f"b_{var}",
                    default=base_defaults.get(var, None),
                    unit=var_units[var],
                    desc=var_descs[var]
                )
                input_params[var] = val
        ...
        for i, var in enumerate(variables["s"]):
            ...
            with cols[i % 3]:
                ...
                val = get_input_streamlit(
                    ...
                    var_name=var,
                    key=f"s_{var}",
                    default=sample_defaults.get(var, None),
                    unit=var_units[var],
                    desc=var_descs[var]
                )
                input_params[var] = val

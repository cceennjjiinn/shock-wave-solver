...
def validate_physical合理性(data, material_type="通用"):
    ...
    if 'P' in data and 'rho0' in data and 'Us' in data and 'Up' in data:
        if None not in [data['P'], data['rho0'], data['Us'], data['Up']]:
            ...
            if abs(data['P'] - calculated_P) > 0.1 * calculated_P:  # 允许10%误差
    ...
    if 'rho' in data and 'rho0' in data and data['rho'] is not None and data['rho0'] is not None:
        if data['rho'] <= data['rho0'] - 1e-6:  # 考虑浮点数精度
            errors.append(f"{material_type}压缩密度(rho={data['rho']})必须大于初始密度(rho0={data['rho0']})")
    
    # 比体积比检查：必须小于1
    if 'V_V0' in data and data['V_V0'] is not None and data['V_V0'] >= 1 + 1e-6:
        errors.append(f"{material_type}比体积比(V/V0={data['V_V0']})必须小于1")
    
    # 温度检查：冲击温度应高于室温
    if 'T' in data and data['T'] is not None and data['T'] < 200:  # 低于200K视为异常
        errors.append(f"{material_type}冲击温度(T={data['T']})异常低，应高于室温(约300K)")
    
    # 格吕奈森系数检查：通常在0.5到5之间
    if 'gamma' in data and data['gamma'] is not None:
        if data['gamma'] <= 0 or data['gamma'] > 10:
            errors.append(f"{material_type}格吕奈森系数(gamma={data['gamma']})应在0到10之间")
...
def calculate_shock_parameters(U_s, u_p, rho0, gamma=2.0, Cv=385, T0=300, calculate_temp=True):
    ...
    # 检查计算结果的物理合理性
    if rho <= rho0:
        raise ValueError(f"计算得到的压缩密度(rho={rho})必须大于初始密度(rho0={rho0})")
    if V_V0 >= 1:
        raise ValueError(f"计算得到的比体积比(V/V0={V_V0})必须小于1")
    if P <= 0:
        raise ValueError(f"计算得到的压力(P={P})必须为正数")
    ...
    if calculate_temp:
        ...
        if T < T0:
            raise ValueError(f"计算得到的冲击温度(T={T})必须高于初始温度(T0={T0})")
...
def fit_hugoniot(df):
    ...
    if C0 <= 0:
        st.warning(f"Hugoniot拟合得到的体声速(C0={C0})为非正数，已调整为合理值")
        C0 = max(1.0, abs(C0))  # 确保体声速为正数且合理
        
    if S < 1.0 or S > 3.0:
        st.warning(f"Hugoniot参数(S={S})超出典型范围(1.0-3.0)，可能数据存在问题")
...
def fit_material_data(df, material_name, material_type):
    ...
    # 物理约束检查
    if C0 <= 0:
        st.warning(f"{material_type}材料 '{material_name}' 拟合得到的体声速(C0={C0})为非正数，已调整")
        C0 = max(1.0, abs(C0))
        
    if S < 1.0 or S > 3.0:
        st.warning(f"{material_type}材料 '{material_name}' Hugoniot参数(S={S})超出典型范围(1.0-3.0)")
...
def generate_shock_plots(df, C0, S, material_name, material_type):
    ...
    # 添加物理约束线：Us = Up（冲击波速度必须大于粒子速度）
    axs[0, 0].plot(u_p_range, u_p_range, 'k--', label='Us = Up (物理边界)')
    ...
    axs[1, 0].axvline(x=1.0, color='k', linestyle='--', label='V/V0 = 1 (物理边界)')
    ...
    # 添加初始密度参考线
    axs[1, 1].axhline(y=rho0, color='k', linestyle='--', label=f'初始密度: {rho0:.2f} g/cm³')
...
def plot_results_streamlit(results, calculate_temp=True):
    ...
    ax3.plot([0, max_val], [0, max_val], 'k--', label='Us = Up (物理边界)')
...
def database_mode_page():
    ...
    if st.button("开始求解"):
        ...
        if results:
            ...
            st.warning(f"未找到有效解，共尝试 {total} 组参数，均不符合物理规律或求解失败")
...
def manual_mode_page():
    ...
    col1, col2, col3 = st.columns(3)
    with col1:
        flyer_material = st.text_input("飞片材料名称", value="Copper", help="输入材料名称，例如: Copper, Aluminum")
    with col2:
